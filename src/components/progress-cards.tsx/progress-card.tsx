"use client";
import React from "react";
import { CardsArrayProps } from "./progress-cards";
import { SignupIcon } from "../../icons/svg";
import { useTranslation } from "react-i18next";
import { useTheme } from "next-themes";
import Image from "next/image";

const ProgressCard: React.FC<CardsArrayProps> = (props) => {
  const { resolvedTheme: isDark } = useTheme();
  const { t, i18n } = useTranslation();
  const direction =
    i18n.dir() === "rtl" || i18n.language === "ar" ? "rtl" : "ltr";
  return (
    <div
      style={{ direction }}
      className="grid grid-cols-1 lg:grid-cols-2 gap-x-5 gap-y-1 md:gap-y-6"
    >
      <div className="lg:order-last flex justify-center lg:block">
        <Image
          className="dark:hidden block"
          width={522}
          height={668}
          src={props.lightImageUrl}
          alt=""
        />
        <Image
          className="hidden dark:block"
          width={522}
          height={668}
          src={props.darkImageUrl}
          alt=""
        />
      </div>
      {props.title === t("signup") ? (
        <div className="flex flex-col gap-y-2 md:order-1 mt-3 lg:mt-0">
          <div
            className={`
               dark:lg:bg-[#FFFFFF0A]  lg:bg-[#F6F6F6]
             lg:w-[450px] sm:px-[33px] py-[24px] lg:mt-[50px]`}
          >
            <div className="flex items-center gap-x-3">
              {props.icon}
              <p
                className={`text-[18px] sm:text-[22px] font-bold 
                   dark:text-white text-opacity-[63%]`}
              >
                {props.title}
              </p>
            </div>
            <div className="sm:mt-2">
              <p
                className={`
                   dark:text-[#B5B5B5] text-[#717171]
                 text-[12px] sm:text-[14px] leading-5`}
              >
                {props.desc}
              </p>
            </div>
          </div>
          <div className="hidden lg:flex flex-col gap-y-[60px] mt-[46px] px-[33px]">
            <div className="flex items-center gap-x-3">
              <Image
                src="/images/gateway-setup-icon.png"
                width={22}
                height={24}
                alt=""
                className="hidden dark:block"
              />
              <Image
                src="/images/gateway-setup-icon-dark.png"
                width={22}
                height={24}
                alt=""
                className="dark:hidden block"
              />
              <p
                className={`text-[22px] text-[#12121296] font-bold 
                 dark:text-[#636365] text-opacity-[36%]`}
              >
                {t("gatewaySetup")}
              </p>
            </div>
            <div className="flex items-center gap-x-3">
              <Image
                src="/images/api-creation-dark.png"
                width={22}
                height={24}
                alt=""
                className="hidden dark:block"
              />
              <Image
                src="/images/api-creation-light.png"
                width={22}
                height={24}
                alt=""
                className="dark:hidden block"
              />
              <p
                className={`text-[22px] text-[#12121296] font-bold 
               dark:text-[#636365] text-opacity-[36%]`}
              >
                {t("progressThree")}
              </p>
            </div>
            <div className="flex items-center gap-x-3">
              <Image
                src="/images/settlement-request-dark-icon.png"
                width={22}
                height={24}
                alt=""
                className="hidden dark:block"
              />
              <Image
                src="/images/settlement-request-light-icon.png"
                width={22}
                height={24}
                alt=""
                className="dark:hidden block"
              />
              <p
                className={`text-[22px] text-[#12121296] font-bold 
                dark:text-[#636365] text-opacity-[36%]`}
              >
                {t("settlementRequests")}
              </p>
            </div>
            <div className="flex items-center gap-x-3">
              <Image
                src="/images/security-measures-dark-icon.png"
                width={22}
                height={24}
                alt=""
                className="hidden dark:block"
              />
              <Image
                src="/images/security-measures-light-icon.png"
                width={22}
                height={24}
                alt=""
                className="dark:hidden block"
              />
              <p
                className={`text-[22px] text-[#12121296] font-bold 
                dark:text-[#636365] text-opacity-[36%]`}
              >
                {t("securityMeasures")}
              </p>
            </div>
          </div>
          <div className="mt-[40px] lg:hidden">
            <hr className="border-t-1 dark:border-[#FFFFFF14] border-[#ececec]" />
          </div>
        </div>
      ) : props.title === t("gatewaySetup") ? (
        <div className="flex flex-col gap-y-2 order-1 mt-3 md:mt-0">
          <div className="hidden lg:flex items-center gap-x-3 px-[33px] md:mt-[50px]">
            <SignupIcon color={isDark === "dark" ? "#FFFFFF5C" : undefined} />
            <p
              className={`text-[22px] text-[#12121296] font-bold 
                dark:text-[#FFFFFF5C] text-opacity-[36%]`}
            >
              {t("signup")}
            </p>
          </div>
          <div
            className={`
           dark:lg:bg-[#FFFFFF0A]  lg:bg-[#F6F6F6]
         lg:w-[450px] sm:px-[33px] py-[24px] lg:mt-[50px]`}
          >
            <div
              className={`text-[18px] sm:text-[22px] font-bold 
                dark:text-white 
               flex items-center gap-x-3`}
            >
              {props.icon}
              <p
                className={`text-[18px] sm:text-[22px] font-bold 
                dark:text-white`}
              >
                {props.title}
              </p>
            </div>
            <div className="sm:mt-2">
              <p
                className={`
                  dark:text-[#B5B5B5] text-[#717171]
                text-[12px] sm:text-[14px] leading-5`}
              >
                {props.desc}
              </p>
            </div>
          </div>
          <div className="hidden lg:flex flex-col gap-y-[60px] mt-[46px] sm:mt-0 px-[33px]">
            <div className="flex items-center gap-x-3 mt-[25px]">
              <Image
                src="/images/api-creation-dark.png"
                width={22}
                height={24}
                alt=""
                className="hidden dark:block"
              />
              <Image
                src="/images/api-creation-light.png"
                width={22}
                height={24}
                alt=""
                className="dark:hidden block"
              />
              <p
                className={`text-[22px] text-[#12121296] font-bold 
                  dark:text-[#FFFFFF5C] text-opacity-[36%]
                `}
              >
                {t("progressThree")}
              </p>
            </div>
            <div className="flex items-center gap-x-3">
              <Image
                src="/images/settlement-request-dark-icon.png"
                width={22}
                height={24}
                alt=""
                className="hidden dark:block"
              />
              <Image
                src="/images/settlement-request-light-icon.png"
                width={22}
                height={24}
                alt=""
                className="dark:hidden block"
              />
              <p
                className={`text-[22px] text-[#12121296] font-bold 
               dark:text-[#FFFFFF5C] text-opacity-[36%]
             `}
              >
                {t("settlementRequests")}
              </p>
            </div>
            <div className="flex items-center gap-x-3">
              <Image
                src="/images/security-measures-dark-icon.png"
                width={22}
                height={24}
                alt=""
                className="hidden dark:block"
              />
              <Image
                src="/images/security-measures-light-icon.png"
                width={22}
                height={24}
                alt=""
                className="dark:hidden block"
              />
              <p
                className={`text-[22px] text-[#12121296] font-bold 
                dark:text-[#FFFFFF5C] text-opacity-[36%]
              `}
              >
                {t("securityMeasures")}
              </p>
            </div>
          </div>
          <div className="mt-[40px] lg:hidden">
            <hr className="border-t-1 dark:border-[#FFFFFF14] border-[#ececec]" />
          </div>
        </div>
      ) : props.title === t("progressThree") ? (
        <div className="flex flex-col gap-y-2 order-1 mt-3 md:mt-0">
          <div className="hidden lg:flex flex-col gap-y-[60px]">
            <div className="hidden lg:flex items-center gap-x-3 px-[33px] mt-[50px]">
              <SignupIcon color={isDark === "dark" ? "#FFFFFF5C" : undefined} />
              <p
                className={`text-[22px] text-[#12121296] font-bold 
                dark:text-[#FFFFFF5C] text-opacity-[36%]
              `}
              >
                {t("signup")}
              </p>
            </div>
            <div className="hidden lg:flex items-center gap-x-3 px-[33px]">
              <Image
                src="/images/gateway-setup-icon.png"
                width={22}
                height={24}
                alt=""
                className="hidden dark:block"
              />
              <Image
                src="/images/gateway-setup-icon-dark.png"
                width={22}
                height={24}
                alt=""
                className="dark:hidden block"
              />
              <p
                className={`text-[22px] text-[#12121296] font-bold 
               dark:text-[#FFFFFF5C] text-opacity-[36%]
             `}
              >
                {t("gatewaySetup")}
              </p>
            </div>
          </div>
          <div
            className={`
      dark:lg:bg-[#FFFFFF0A]  lg:bg-[#F6F6F6]
    lg:w-[450px] sm:px-[33px] py-[24px] lg:mt-[50px]`}
          >
            <div className="flex items-center gap-x-3">
              {props.icon}
              <p
                className={`text-[18px] sm:text-[22px] font-bold 
                  dark:text-white`}
              >
                {props.title}
              </p>
            </div>
            <div className="sm:mt-2">
              <p
                className={`
                  dark:text-[#B5B5B5] text-[#717171]
                 text-[12px] sm:text-[14px] leading-5`}
              >
                {props.desc}
              </p>
            </div>
          </div>
          <div className="hidden lg:flex flex-col gap-y-[60px] mt-[46px] sm:mt-0 px-[33px]">
            <div className="flex items-center gap-x-3 mt-[25px]">
              <Image
                src="/images/settlement-request-dark-icon.png"
                width={22}
                height={24}
                alt=""
                className="hidden dark:block"
              />
              <Image
                src="/images/settlement-request-light-icon.png"
                width={22}
                height={24}
                alt=""
                className="dark:hidden block"
              />
              <p
                className={`text-[22px] text-[#12121296] font-bold 
               dark:text-[#FFFFFF5C] text-opacity-[36%]
             `}
              >
                {t("settlementRequests")}
              </p>
            </div>
            <div className="flex items-center gap-x-3">
              <Image
                src="/images/security-measures-dark-icon.png"
                width={22}
                height={24}
                alt=""
                className="hidden dark:block"
              />
              <Image
                src="/images/security-measures-light-icon.png"
                width={22}
                height={24}
                alt=""
                className="dark:hidden block"
              />
              <p
                className={`text-[22px] text-[#12121296] font-bold 
                dark:text-[#FFFFFF5C] text-opacity-[36%]
              `}
              >
                {t("securityMeasures")}
              </p>
            </div>
          </div>
          <div className="mt-[40px] lg:hidden">
            <hr className="border-t-1 dark:border-[#FFFFFF14] border-[#ececec]" />
          </div>
        </div>
      ) : props.title === t("settlementRequests") ? (
        <div className="flex flex-col gap-y-2 order-1 mt-3 md:mt-0">
          <div className="hidden lg:flex flex-col gap-y-[60px]">
            <div className="hidden lg:flex items-center gap-x-3 px-[33px] md:mt-[50px]">
              <SignupIcon />
              <p
                className={`text-[22px] text-[#12121296] font-bold 
                  dark:text-[#FFFFFF5C] text-opacity-[36%]
                }`}
              >
                Sign Up
              </p>
            </div>
            <div className="hidden lg:flex items-center gap-x-3 px-[33px]">
              <Image
                src="/images/gateway-setup-icon.png"
                width={22}
                height={24}
                alt=""
                className="hidden dark:block"
              />
              <Image
                src="/images/gateway-setup-icon-dark.png"
                width={22}
                height={24}
                alt=""
                className="dark:hidden block"
              />
              <p
                className={`text-[22px] text-[#12121296] font-bold 
                dark:text-[#FFFFFF5C] text-opacity-[36%]
              }`}
              >
                Gateway Setup
              </p>
            </div>
            <div className="hidden lg:flex items-center gap-x-3 px-[33px]">
              <Image
                src="/images/api-creation-dark.png"
                width={22}
                height={24}
                alt=""
                className="hidden dark:block"
              />
              <Image
                src="/images/api-creation-light.png"
                width={22}
                height={24}
                alt=""
                className="dark:hidden block"
              />
              <p
                className={`text-[22px] text-[#12121296] font-bold 
                 dark:text-[#FFFFFF5C] text-opacity-[36%]
               }`}
              >
                API or Paylink Creation
              </p>
            </div>
          </div>
          <div
            className={`
         dark:lg:bg-[#FFFFFF0A]  lg:bg-[#F6F6F6]
       lg:w-[450px] sm:px-[33px] py-[24px] lg:mt-[50px]`}
          >
            <div className="flex items-center gap-x-3">
              {props.icon}
              <p
                className={`text-[18px] sm:text-[22px] font-bold
                  dark:text-white
                }`}
              >
                {props.title}
              </p>
            </div>
            <div className="sm:mt-2">
              <p
                className={`
                   dark:text-[#B5B5B5] text-[#717171]
                 text-[12px] sm:text-[14px] leading-5`}
              >
                {props.desc}
              </p>
            </div>
          </div>
          <div className="hidden lg:flex flex-col gap-y-10 mt-[46px] sm:mt-0 px-[33px]">
            <div className="flex items-center gap-x-3 mt-[25px]">
              <Image
                src="/images/security-measures-dark-icon.png"
                width={22}
                height={24}
                alt=""
                className="hidden dark:block"
              />
              <Image
                src="/images/security-measures-light-icon.png"
                width={22}
                height={24}
                alt=""
                className="dark:hidden block"
              />
              <p
                className={`text-[22px] text-[#12121296] font-bold 
                dark:text-[#FFFFFF5C] text-opacity-[36%]
              }`}
              >
                Security Measures
              </p>
            </div>
          </div>
          <div className="mt-[40px] lg:hidden">
            <hr className="border-t-1 dark:border-[#FFFFFF14] border-[#ececec]" />
          </div>
        </div>
      ) : props.title === t("securityMeasures") ? (
        <div className="flex flex-col gap-y-2 order-1 mt-3 md:mt-0">
          <div className="hidden lg:flex flex-col gap-y-[60px]">
            <div className="hidden lg:flex items-center gap-x-3 px-[33px] md:mt-[50px]">
              <SignupIcon color={isDark === "dark" ? "#636365" : undefined} />
              <p
                className={`text-[22px] text-[#12121296] font-bold 
                 dark:text-[#FFFFFF5C] text-opacity-[36%]
               }`}
              >
                {t("signup")}
              </p>
            </div>
            <div className="hidden lg:flex items-center gap-x-3 px-[33px]">
              {/* <GatewaySetupIcon
                color={isDark === "dark" ? "#FFFFFF5C" : undefined}
              /> */}
              <Image
                src="/images/gateway-setup-icon.png"
                width={22}
                height={24}
                alt=""
                className="hidden dark:block"
              />
              <Image
                src="/images/gateway-setup-icon-dark.png"
                width={22}
                height={24}
                alt=""
                className="dark:hidden block"
              />
              <p
                className={`text-[22px] text-[#12121296] font-bold 
                 dark:text-[#FFFFFF5C] text-opacity-[36%]
               }`}
              >
                {t("gatewaySetup")}
              </p>
            </div>
            <div className="hidden lg:flex items-center gap-x-3 px-[33px]">
              <Image
                src="/images/api-creation-dark.png"
                width={22}
                height={24}
                alt=""
                className="hidden dark:block"
              />
              <Image
                src="/images/api-creation-light.png"
                width={22}
                height={24}
                alt=""
                className="dark:hidden block"
              />
              <p
                className={`text-[22px] text-[#12121296] font-bold 
                dark:text-[#FFFFFF5C] text-opacity-[36%]
              }`}
              >
                {t("progressThree")}
              </p>
            </div>
            <div className="hidden lg:flex items-center gap-x-3 px-[33px]">
              <Image
                src="/images/settlement-request-dark-icon.png"
                width={22}
                height={24}
                alt=""
                className="hidden dark:block"
              />
              <Image
                src="/images/settlement-request-light-icon.png"
                width={22}
                height={24}
                alt=""
                className="dark:hidden block"
              />
              <p
                className={`text-[22px] text-[#12121296] font-bold 
                 dark:text-[#FFFFFF5C] text-opacity-[36%]
               }`}
              >
                {t("settlementRequests")}
              </p>
            </div>
          </div>
          <div
            className={`
         dark:lg:bg-[#FFFFFF0A]  lg:bg-[#F6F6F6]
       lg:w-[450px] sm:px-[33px] py-[24px] lg:mt-[50px]`}
          >
            <div className="flex items-center gap-x-3">
              {props.icon}
              <p
                className={`text-[18px] sm:text-[22px] font-bold
                  dark:text-white 
                }`}
              >
                {props.title}
              </p>
            </div>
            <div className="sm:mt-2">
              <p
                className={`
                   dark:text-[#B5B5B5]  text-[#717171]
                text-[12px] sm:text-[14px] leading-5`}
              >
                {props.desc}
              </p>
            </div>
          </div>
          <div className="mt-[40px] lg:hidden">
            <hr className="border-t-1 dark:border-[#FFFFFF14] border-[#ececec]" />
          </div>
        </div>
      ) : null}
    </div>
  );
};

export default ProgressCard;
